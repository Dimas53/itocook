...
**Why this exists:** Without forks, editing a shared recipe would affect all cooks who use it. With forks, each cook gets their own version that they can modify freely. The `dedupRecipes` utility ensures only one version per dish name is shown in recipe lists (preferring forks over originals).
...
**Visibility change sync** — A `document.visibilitychange` listener re-fetches queue entry status when the browser tab becomes visible, so the cook sees admin changes without manual refresh.
...
- **Double fork prevention** — `saveDish()` checks for existing forks by the same user for the same original recipe before creating a new one.
- **Ghost participants** — Users who joined but didn't show up or cancel late are not yet penalized (deferred to Phase 6).
