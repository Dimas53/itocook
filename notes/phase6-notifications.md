...
Это называется auto-cleanup и это стандартная практика.
Самый простой вариант — удалять прочитанные уведомления старше 24 часов. Логика:

Прочитал → висит ещё день → удаляется автоматически
Не прочитал → остаётся пока не прочитаешь (или максимум 7 дней)

Реализуется одним Directus Flow по расписанию — например каждую ночь в 23:00:
Cron: 0 23 * * *
→ Delete Items из notifications где:
(read = true AND date_created < now - 24h)
OR
(date_created < now - 7 days)  ← страховка от накопления
Это лучше чем "удалять если касается сегодня" — потому что сложно определить к какому дню относится уведомление про баланс например.
Но это не срочно — можно добавить в конце Phase 6 как Step 4.5 или просто вручную чистить пока идёт разработка. Сейчас важнее починить Cook Assigned и протестировать Lunch Ready.
